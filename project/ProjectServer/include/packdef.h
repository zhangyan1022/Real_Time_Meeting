...
typedef enum Net_PACK
{
    ...
    DEF_PACK_SET_USER_INFO,

    DEF_PACK_SET_UPLOAD_RQ,
    DEF_PACK_SET_UPLOAD_RS,
    DEF_PACK_SET_FILEBLOCK_RQ,
}Net_PACK;
...
struct STRU_SET_USER_INFO
{
    STRU_SET_USER_INFO()
    {
        m_nType = DEF_PACK_SET_USER_INFO;
        m_UserID = 0;
        m_iconid = 0;
...
struct FileInfo
{
    ...
    int userID; //添加用户id 方便后续传输
    int friendID;//添加friendid 方便后续传输
    FILE *pFile;
};
/*
//.上传文件请求
typedef struct STRU_UPLOAD_RQ
{
    ...
    STRU_UPLOAD_RQ()
    {
        m_nType = DEF_PACK_SET_UPLOAD_RQ;
        m_UserId = 0;
        m_friendId = 0;
        m_nFileSize = 0;
...
typedef struct STRU_UPLOAD_RS
{
    STRU_UPLOAD_RS()
    {
        m_nType = DEF_PACK_SET_UPLOAD_RS;
        m_UserId= 0 ;
        m_friendId = 0;
        memset( m_szFileMD5 , 0 , MAX_SIZE);
        m_nResult = 0;
...
typedef struct STRU_FILEBLOCK_RQ
{
    STRU_FILEBLOCK_RQ()
    {
        m_nType = DEF_PACK_SET_FILEBLOCK_RQ;
        m_nUserId = 0;
        m_friendId = 0;
        memset( m_szFileMD5 , 0 , MAX_SIZE);
        m_nBlockLen =0;
